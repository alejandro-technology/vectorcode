...
fn chunk_typescript_fixture() {
    ...
    assert!(!chunks.is_empty(), "Should produce chunks from TypeScript fixture");
    assert!(chunks.len() >= 2, "Should produce at least 2 chunks (class + function), got {}", chunks.len());
    ...
    let has_class = chunks.iter().any(|c| {
        c.kind == "export_statement" && c.content.contains("Calculator")
    });
...
fn chunk_typescript_task_fixture() {
    ...
    assert!(!chunks.is_empty(), "Should produce chunks from TypeScript task fixture");
    
    // Verify we have multiple chunks (interface, enum, functions)
    assert!(chunks.len() >= 3, "Should produce at least 3 chunks, got {}", chunks.len());
}
...
fn chunk_python_fixture() {
    ...
    assert!(!chunks.is_empty(), "Should produce chunks from Python fixture");
    assert!(chunks.len() >= 2, "Should produce at least 2 chunks (User class + AuthService class), got {}", chunks.len());
    ...
    let has_auth_service = chunks.iter().any(|c| c.content.contains("class AuthService"));
    assert!(has_user_class || has_auth_service, "Should have extracted at least one class");
}
...
fn chunk_python_pipeline_fixture() {
    ...
    assert!(!chunks.is_empty(), "Should produce chunks from Python pipeline fixture");
...
fn chunk_rust_fixture() {
    ...
    assert!(!chunks.is_empty(), "Should produce chunks from Rust fixture");
    assert!(chunks.len() >= 1, "Should produce at least 1 chunk (impl), got {}", chunks.len());
...
fn chunk_rust_http_fixture() {
    ...
    assert!(!chunks.is_empty(), "Should produce chunks from Rust HTTP fixture");
    
    // Verify we have multiple chunks (enums, structs, traits, impls)
    assert!(chunks.len() >= 5, "Should produce at least 5 chunks, got {}", chunks.len());
}
...
fn chunk_idempotency() {
    ...
    assert_eq!(chunks1.len(), chunks2.len(), "Chunking should be deterministic");
    
    for (c1, c2) in chunks1.iter().zip(chunks2.iter()) {
        assert_eq!(c1.id, c2.id, "Chunk IDs should be identical across runs");
        assert_eq!(c1.content_hash, c2.content_hash, "Content hashes should be identical");
...
fn chunk_different_files_produce_different_ids() {
    ...
    for (c1, c2) in chunks1.iter().zip(chunks2.iter()) {
        assert_ne!(c1.id, c2.id, "Different file paths should produce different chunk IDs");
    }
}
