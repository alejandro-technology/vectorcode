...
4. `roadmap_vectorcode.md:35-38` (the "Estado actual" block, snapshot from the start of Fase 1) listed 4 features as `❌` that are all `✅` today (BM25/FTS, knowledge graph, multi-repo, formal benchmark).
5. `roadmap_vectorcode.md:85` (row 3.3) said "Hashing de archivos (SHA-256)"; the code uses `blake3` (`src/types.rs:127`, `src/types.rs:134`).
...
| 4 features marked `❌` in "Estado actual" | `roadmap_vectorcode.md:35-38` | All 4 are `✅` (Fase 1 + 2 + 3 shipped) | [phase-4.3 commit C2](../../roadmap_vectorcode.md) |
| "Hashing de archivos (SHA-256)" | `roadmap_vectorcode.md:85` (row 3.3) | `blake3` (`src/types.rs:127`, `src/types.rs:134`) | [phase-4.3 commit C2](../../roadmap_vectorcode.md) |
